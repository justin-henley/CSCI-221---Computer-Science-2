...
Fraction::Fraction(int a, int b)
{
    ...
    // Check for negative denominator and move negative to numerator
    if (den < 0)
    {
        num *= -1;
        den *= -1;
...
{
    // Suggestion from clang to clean up return
    return {den, num};
}
...
Fraction operator+(const Fraction &f1, const Fraction &f2)
{
    ...
    // Cross multiplies first to make up for potentially different denominators
...
Fraction operator-(const Fraction &f1, const Fraction &f2)
{
    // Relies on the previous definition of negation and addition
    return f1 + (-f2);
}
...
{
    // Variables for each piece of the fraction
    int num, den;
    char slash;
    ...
    //Set the fraction f with the new values
    f = Fraction(num, den);
